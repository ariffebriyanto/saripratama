using System;
using System.Collections.Generic;
using System.Text;

namespace IFA.Domain.Models
{
    public class CETAK_DPB
    {
        public string status_cetak { get; set; }

        public string Tipe_trans { get; set; }

        public string Flag_Ppn { get; set; }

        public string Departement { get; set; }

        public string pending { get; set; }

        public string Nama_Departemen { get; set; }

        public string Kd_Cabang { get; set; }

        public string Kd_Customer { get; set; }

        public string Kd_sales { get; set; }

        public string Nama_Sales { get; set; }

        public string Jenis_sp { get; set; }

        public DateTime? Tgl_Kirim { get; set; }

        public string Nama_Customer { get; set; }

        public string Kd_Wilayah { get; set; }

        public string Nama_Wilayah { get; set; }

        public string Nama_Group_Wilayah { get; set; }

        public string Kode_Area { get; set; }

        public string Kota1 { get; set; }

        public string No_Telepon1 { get; set; }

        public string Alamat1 { get; set; }

        public string Nama_Kota { get; set; }

        public string Nama_Area { get; set; }

        public string Almt_pnrm { get; set; }

        public string SP_Reff { get; set; }

        public DateTime? Tgl_sp { get; set; }

        public string keterangan_krm { get; set; }

        public string No_sp { get; set; }

        public string old_num { get; set; }

        public string no_seq_d { get; set; }

        public string No_sp_box { get; set; }

        public string Kd_Stok { get; set; }

        public string Nama_Barang { get; set; }

        public string Keterangan { get; set; }

        public string Kd_Satuan { get; set; }

        public string Nama_Satuan { get; set; }

        public string kd_jns_persd { get; set; }

        public string kd_jenis { get; set; }

        public string kd_merk { get; set; }

        public string Nama_Jenis { get; set; }

        public string Jenis_Group { get; set; }

        public string Nama_Jenis_Group { get; set; }

        public DateTime? tgl_kirim_SO { get; set; }

        public string prioritas { get; set; }

        public string Status_Inspeksi { get; set; }

        public string Status_Simpan { get; set; }

        public decimal? QtyCetak { get; set; }

        public string bom_service { get; set; }

        public string sts_centi { get; set; }

        public string isClosed { get; set; }

        public decimal vol { get; set; }

        public decimal tebal { get; set; }

        public decimal lebar { get; set; }

        public decimal panjang { get; set; }

        public decimal? qty_order { get; set; }

        public decimal? qty_rcn_prod { get; set; }

        public DateTime? tanggal_prod { get; set; }

        public decimal? qty_rcn_krm { get; set; }

        public decimal? qty_sisa_krm { get; set; }

        public decimal? qty_sisa_prod { get; set; }

        public decimal? qty_balik { get; set; }

        public string Expr1 { get; set; }

        public decimal? sisa { get; set; }

        public decimal nilai { get; set; }

        public decimal nilai_m3 { get; set; }

        public string no_dpb { get; set; }

        public decimal jumlah { get; set; }

        public decimal jml_indeks { get; set; }

        public decimal jml_m3 { get; set; }

        public string rec_stat { get; set; }

        public string program_name { get; set; }

        public string pendingh { get; set; }
    }

    public class inputcetakDPB
    {
        public List<CETAK_DPB> details { get; set; }
    }
}

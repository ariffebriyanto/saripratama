...
namespace IFA.Web.Controllers
{
    ...
    {
        public MOPController(FactoryClass factoryClass,
        ...
        [Authorize(Roles = "Admin, User, UAT, SPV, LOGISTIK")]
        public async Task<string> GetOwnerPartial(string sorting, string filter, int skip, int take, int pageSize, int page, DateTime DateFrom, DateTime DateTo, string owner,int harga,string pet)
        {
            List<SIF_Owner> result = new List<SIF_Owner>();
            ApiClient client = factoryClass.APIClientAccess();
            Response response = new Response();
            try
            {
                response = await client.CallAPIGet("SIF_OWNER/GetOwnerPartial?skip=" + skip + "&take=" + take + "&pageSize=" + pageSize + "&page=" + page + "&DateFrom=" + DateFrom + "&DateTo=" + DateTo + "&sorting=" + sorting + "&filter=" + filter + "&owner=" + owner + "&harga=" + harga + "&owner=" + owner);
        ...
        [Authorize(Roles = "Admin, User, UAT, SPV, LOGISTIK")]
        public async Task<IActionResult> GetOwnerDetail(FilterSIF_Pet filterPET)
        {           
            ...
            try
            {
                response = await client.CallAPIGet("SIF_Pet/GetDetailMPO?kd_owner=" + filterPET.kd_owner +  "&pet=" + filterPET.pet + "&DateFrom=" + filterPET.DateFrom + "&DateTo=" + filterPET.DateTo + "&harga=" + filterPET.harga + "&owner=" + filterPET.owner);

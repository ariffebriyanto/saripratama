...
function getGudang() {
    return $.ajax({
        ...
        success: function (result) {
            $("#gudang").empty();
            $("#gudang").append('<option value="" selected disabled>Pilih Gudang</option>');
            var data = result;
            GudangList = result;
...
function getDataHeader() {
    ...
    return $.ajax({
        url: urlGetDataStokHeader + '?kd_stok=' + barang + "&bulan=" + Bulan + "&tahun=" + Tahun + "&gudang=" + Gudang ,
        type: "GET",
        success: function (result) {
            console.log(result);
            if (result.length>0) {
                ...
                $('#lblGudang').text($("#gudang option:selected").text());
                $("#divHeader").show();
            }
...
function bindGrid() {
    _kartustokGrid = $("#kartustokGrid").kendoGrid({
        columns: [
            { field: "tanggal", title: "Tanggal", width: "60px", template: "#= kendo.toString(kendo.parseDate(tanggal, 'yyyy-MM-dd'), 'dd MMMM yyyy') #" },
            { field: "no_trans", title: "No Bukti", width: "110px" },
            ...
            { field: "userid", title: "User", width: "60px" },
...
function getDataPrint() {
    ...
    var Tahun = $('#Tahun').val();
    var Gudang = $('#gudang').val();
    var gudangName = $("#gudang option:selected").text();

    if ($('#checkbox2').is(":checked")) {
        Bulan = "";
    }
    return $.ajax({
        url: urlGetPrintKartuStok + '?kd_stok=' + barang + "&bulan=" + Bulan + "&tahun=" + Tahun + "&gudang=" + Gudang + "&gudangName=" + gudangName,
        type: "POST",
        success: function (result) {

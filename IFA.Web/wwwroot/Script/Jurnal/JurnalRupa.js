...
columnOpname = [
        ...
        { field: "nama_Customer", title: "Kartu", width: "120px", hidden: true, editor: CustomerDropDown },
        ...
        { field: "saldo_val_debet", title: "Val Debet", width: "120px", editor: hargaNumeric, format: "{0:n2}", decimals: 2, min: 0, attributes: { class: "text-right ", 'style': 'background-color: aquamarine; color:black;' }, aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>"},
    { field: "saldo_val_kredit", title: "Val Kredit", width: "120px", editor: hargaNumeric2, format: "{0:n2}", decimals: 2, min: 0, attributes: { class: "text-right ", 'style': 'background-color: aquamarine; color:black;' }, aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>"},
    { field: "saldo_rp_debet", title: "Rp Debet", width: "120px", format: "{0:n2}", decimals: 2, min: 0, attributes: { class: "text-right ", 'style': 'background-color: darkseagreen; color:black;' }, aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>"},
    { field: "saldo_rp_kredit", title: "Rp Kredit", width: "120px", format: "{0:n2}", decimals: 2, min: 0, attributes: { class: "text-right ", 'style': 'background-color: darkseagreen; color:black;' }, aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>" },
    ...
    if (idOpname != null && idOpname != "") {
        ...
        $.when(getCustomer()).done(function () {
            $.when(fillCboCustomer()).done(function () {
                $.when(getCustomer1()).done(function () {
...
function getData(id) {
    ...
    return $.ajax({
        ...
        { field: "saldo_val_debet", title: "Val Debet", width: "120px", editor: hargaNumeric, format: "{0:n2}", decimals: 2, min: 0, attributes: { class: "text-right ", 'style': 'background-color: aquamarine; color:black;' }, aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>"},
        { field: "saldo_val_kredit", title: "Val Kredit", width: "120px", editor: hargaNumeric2, format: "{0:n2}", decimals: 2, min: 0, attributes: { class: "text-right ", 'style': 'background-color: aquamarine; color:black;' }, aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>" },
        { field: "saldo_rp_debet", title: "Rp Debet", width: "120px", aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>" },
        { field: "saldo_rp_kredit", title: "Rp Kredit", width: "120px", aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>" },
        { command: ["destroy"], title: "Actions", width: "40px" }
    ];
                ...
                if (result[0].detail.length > 0) {
                    for (var i = 0; i <= result[0].detail.length - 1; i++) {
                        ...
                        Opnameds.push({
                            ...
                            nama_customer: result[0].detail[i].nama_customer,
...
function hargaNumeric(container, options) {
    var input = $('<input id="saldo_val_debet" />');
    ...
    input.kendoNumericTextBox({
        ...
        change: function (e) {
            ...
            barang.saldo_val_debet = value;
            barang.saldo_val_kredit = 0;
            barang.saldo_rp_kredit = 0;
            barang.saldo_rp_debet = value;
            ...
            gridData.forEach(element => {
                ...
                //groupFooters.eq(0).replaceWith(10);
           ...
           // $('#GridStokOpname').kendoGrid('destroy').empty();
            ...
            //grid.trigger("change");
           // grid.trigger("dataBinding", { action: "rebind" });
    ...
    //Opnameds = [];
    //bindGrid();
    //var numerictextboxdiskon = $("#harga").data("kendoNumericTextBox");
    //if (BranchID == "TOKO" || akses_penjualan == "CASH") {
    //    numerictextboxdiskon.readonly();
    //}
}
function hargaNumeric2(container, options) {
    var input = $('<input id="saldo_val_kredit" />');
    input.appendTo(container);
    ...
    input.kendoNumericTextBox({
        ...
        change: function (e) {
            var value = this.value();
            var barang = _GridStokOpname.dataItem($(e.sender.element).closest("tr"));
            barang.saldo_val_kredit = value;
            barang.saldo_val_debet = 0;
            barang.saldo_rp_debet = 0;
            barang.saldo_rp_kredit = value;
...
function getCustomer1() {
    return $.ajax({
        ...
        success: function (result) {
            customerds1 = [];
            customerds1 = result;
            //console.log("ggxx:" + JSON.stringify(customerds1));
        },
        error: function (data) {
            alert('Something Went Wrong');
            startSpinner('loading..', 0);
    ...
    _GridStokOpname = $("#GridStokOpname").kendoGrid({
        ...
        dataSource: {
            ...
            aggregate: [
                ...
                { field: "saldo_rp_debet", aggregate: "sum" },
                { field: "saldo_rp_kredit", aggregate: "sum" }
            ],
        ...
        dataBound: function (e) {
            ...
            gridData.forEach(element => {
                if (parseInt(element.saldo_val_debet) > 0) {
                    element.saldo_rp_debet = element.saldo_val_debet;
                    element.saldo_rp_kredit = 0;
                    element.saldo_val_kredit = 0;


                } else if (parseInt(element.saldo_val_kredit) > 0){
                    element.saldo_rp_kredit = element.saldo_val_kredit;
                    element.saldo_rp_debet = 0;
                    element.saldo_val_debet = 0;
                }
            ...
            setTimeout(function () {
                ...
                for (var i = 0; i <= ds.length - 1; i++) {
                    ...
                    Opnameds.push({
                        ...
                        nama_Customer: ds[i].nama_Customer,
...
function CustomerDropDown(container, options) {
    ...
    var input = $('<input required id="kd_Customer" name="nama_Customer">');
    input.appendTo(container);
    ...
    input.kendoDropDownList({
        ...
        dataValueField: "nama_Customer",
        dataSource: customerds1,
        optionLabel: "Pilih Kartu",
        filter: "contains",
        template: "<span data-id='${data.kd_Customer}' data-custm='${data.nama_Customer}'>${data.nama_Customer}</span>",
        select: function (e) {
            var id = e.item.find("span").attr("data-id");
            var Custmdp = e.item.find("span").attr("data-custm");
            var custmdp = _GridStokOpname.dataItem($(e.sender.element).closest("tr"));
            custmdp.kartu = id;
            custmdp.nama_Customer = Custmdp;
            custmdp.kd_Customer = id;
            custmdp.Nama_Customer = Custmdp;
            custmdp.NAMA_CUSTOMER = Custmdp;
...
function validationPage() {
    var gudang = $('#Kepada').val();
    //var penerima = $('#penerima').val();
    var ItemData = _GridStokOpname.dataSource.data();
    validationMessage = '';
    //if (!gudang) {
    //    validationMessage = validationMessage + 'Kepada harus di pilih.' + '\n';
    //}
    //if (!penerima) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.success === false) {
                        ...
                        if (result.result == "tidaksama") {
                            Swal.fire({
                                type: 'error',
                                title: 'Warning',
                                html: "Jumlah Debet Kredit Harus Sama"
                            });
                        } else {
                            Swal.fire({
                                type: 'error',

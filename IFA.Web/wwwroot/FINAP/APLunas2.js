...
$(document).ready(function () {
    ...
    $.when(
        ...
        if (Mode != "NEW") {
            $.when(getDataAR(idAP)).done(function () {
                //fillForm();
                if (Mode == "VIEW") {
                    //  bindGrid();
                }
    ...
    return $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                $("#Kd_Supplier").append('<option value="' + data[i].kode_Supplier + '">' + data[i].nama_Supplier + '</option>');
            }
    ...
    return $.ajax({
        ...
        success: function (result) {
           ...
           // $('#addNew').hide();
            ...
            $("#Kd_Supplier option[value='" + result[0].kd_kartu + "']").attr("selected", "selected");
            $('#Kd_Supplier').selectpicker('refresh');
...
function onAddNewRow() {
    ...
    //});
    if ($("#Kd_Supplier").val() != null && $("#Kd_Supplier").val() != "") {
        startSpinner('Loading..', 1);
        $.when(GetInvoice()).done(function () {
            var grid = $("#GridStokOpname").data("kendoGrid");
            grid.addRow();
...
function onPrintTokoClicked() {
    ...
    var namacustomer = $("#Kd_Supplier option:selected").text();
...
function SaveData() {
    ...
    var savedata = {
        ...
        Atas_Nama: $("#Kd_Supplier option:selected").text(),
        Tgl_sp: $("#tanggal").val(),

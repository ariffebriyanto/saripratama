...
function bindGrid() {
    _gvList = $("#gvList").kendoGrid({
        columns: [
            ...
            { field: "nama_supplier", title: "Nama Supplier", width: "250px" },
            {
                field: "sisa"
                , filterable: {
                    extra: true,
                ...
                }, title: "Sisa Utang", width: "60px", format: "{0:#,0.00}", attributes: { class: "text-right " }
            }
        ],
        filterable: {
            extra: false,
...
function fillCboCustomer() {
    ...
    console.log(JSON.stringify(customerds));

    for (var i = 0; i < data.length; i++) {
        $("#Kd_Customer").append('<option value="' + data[i].kode_Supplier + '">' + data[i].nama_Supplier + '</option>');
    }
...
function onChange() {
    ...
    var kd_cust = selectedItem["kd_supplier"];
    startSpinner('Loading..', 1);
    $.ajax({
...
function bindDetailGrid() {
    _gvDetail = $("#gvDetail").kendoGrid({
        ...
        columns: [
            ...
            { field: "jml_tagihan", title: "Jumlah Utang", format: "{0:#,0.00}", attributes: { class: "text-right " }, aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>" },
            ...
            { field: "jml_akhir", title: "Sisa Utang", format: "{0:#,0.00}", attributes: { class: "text-right " }, aggregates: ["sum"], footerTemplate: "<div align=right>#= kendo.toString(sum, \"n2\") #</div>" },
